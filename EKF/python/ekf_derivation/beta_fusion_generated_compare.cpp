...
int main()
{
    ...
    // innovation variance
    beta_innov_var = (HK33*HK39*HK40 + HK34*HK39*HK49 + HK35*HK39*HK50 - HK36*HK39*HK51 + HK37*HK38*HK39 + HK42*HK43 - HK43*HK47 + HK45*HK46 - HK46*HK48 + R_BETA);

    HK52 = HK16 / beta_innov_var;
    ...
    Hfusion[22] = HK29;				// state index 22
    Hfusion[23] = HK16*HK32;		// state index 23
    ...
    for (unsigned row = 7; row <= 9; row++) {
        Kfusion(row) = HK52*(-HK28*P(4,row) + HK28*P(row,22) - HK32*P(5,row) + HK32*P(row,23) + HK33*P(6,row) + HK34*P(2,row) + HK35*P(1,row) - HK36*P(3,row) + HK37*P(0,row));
    }

    for (unsigned row = 10; row <= 15; row++) {
    ...
    for (int row=0; row<24; row++) {
        float diff_fraction;
        if (H_BETA(row) != 0.0f) {
            diff_fraction = fabsf(Hfusion_sympy[row] - H_BETA(row)) / fabsf(H_BETA(row));
        } else if (Hfusion_sympy[row] != 0.0f) {
            diff_fraction = fabsf(Hfusion_sympy[row] - H_BETA(row)) / fabsf(Hfusion_sympy[row]);
        } else {
            diff_fraction = 0.0f;
        }
